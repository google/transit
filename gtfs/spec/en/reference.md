...
* **Sub-journey** - One leg, or multiple legs connected by transfers, that comprise a subset of a journey.
...
| `fare_transfer_type` | Enum | **Conditionally Required** | Indicates the cost processing method of transferring between legs in a journey: <br>![](https://user-images.githubusercontent.com/62957380/144116057-d5cf7ef0-1a67-4e65-922c-10140e410459.png) <br>Valid options are:<br>`0` - `fare_leg_rules.amount` plus the cost of `fare_transfer_rules.amount`; A + AB.<br>`1` - `fare_leg_rules.amount` plus the cost of `fare_transfer_rules.amount` plus the cost of `fare_leg_rules.amount` for the transferred leg; A + AB + B.<br>`2` - The leg with the most expensive `fare_leg_rules.amount` plus the cost of `fare_transfer_rules.amount` ; MAX(A, B) + AB.<br>`3` - `fare_transfer_rules.amount`; AB. <br><br>Cost processing interactions between multiple transfers in a journey:<br>![](https://user-images.githubusercontent.com/62957380/144116095-8a0aedae-5235-4782-a58f-e0b3644dd5f2.png)<br><table><thead><tr><th>`fare_transfer_type`</th><th>Processing A > B</th><th>Processing B > C</th></tr></thead><tbody><tr><td>`0`</td><td>A + AB</td><td>S + BC</td></tr><tr><td>`1`</td><td>A + AB +B</td><td>S + BC + C</td></tr><tr><td>`2`</td><td>MAX(A, B) + AB</td><td>S + MAX(B, C) + BC</td></tr><tr><td>`3`</td><td>AB</td><td>S + BC</td></tr></tbody></table><br>Where S indicates the total calculated cost of the preceding leg(s) and transfer(s).<br><br>Conditionally Required:<br>- **Required** if `fare_transfer_rules.amount` is defined.<br>- **Forbidden** otherwise. |
...
| `currency` | Currency code | **Conditionally Required** | **Conditionally Required** | The currency of the cost of the transfer rule.<br><br>Conditionally Required:<br>- **Required** if `fare_transfer_rules.amount` is defined.<br>- **Forbidden** otherwise. |
