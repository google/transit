...
5.  [Field Definitions](#field-definitions)
    ...
    -   [direction\_names.txt](#direction_namestxt)
...
|  [direction_names.txt](#direction_namestxt) | Optional | Provides direction names for wayfinding. |
...
|  `direction_name_id| ID referencing `direction_names.direction_name_id` | Optional | Defines a direction name for the trip. If the `direction_name` changes during a trip, `trips.direction_name_id` can be overwritten by specifying values for `stop_times.direction_name_id`. <br><br>A `direction_name` should not replicate values provided by the fields `trips.trip_headsign` and `stop_times.stop_headsign`, but must be complementary with these fields (e.g. `direction_name` "Westbound" with `trip_headsign` "Hillsboro"). |
...
|  `direction_name_id` | ID referencing `direction_names.direction_name_id` | Optional | Defines a direction name from this `stop_time` to the subsequent one. This field overrides `trips.direction_name_id` when the `direction_name` changes between this `stop_time` and the subsequent one. If the `direction_name` is displayed for an entire trip, use `trips.direction_name_id` instead. <br><br>A `direction_name_id` value specified for one `stop_time` does not apply to subsequent `stop_time`s in the same trip. If you want to override the `trips.direction_name_id` for multiple `stop_time`s in the same trip, the `stop_times.direction_name_id` must be repeated in each `stop_time` row. If a `direction_name_id` is not provided for subsequent `stop_time`s, the direction shown to riders will default back to the one defined in `trips.direction_name_id` for the trip.<br><br>A `direction_name` must not replicate values provided in the fields `trips.trip_headsign` and `stop_times.stop_headsign`, but must be complementary with these fields (e.g. `direction_name` "Westbound" with `trip_headsign` "Hillsboro"). |
...
| Field Name | Type | Required | Description |
| ------ | ------ | ------ | ------ |
| `direction_name_id` | ID | **Required** | Identifies a direction name |
| `direction_name` | Text | **Required** | Defines a name for the direction used by the transit agency in it's wayfinding system to identify the direction of travel (e.g. North, Southbound, Inbound, Clockwise). |
