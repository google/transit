...
| `instructions`| Text | Optional | Instructions useful for riders when the pathway is taken from `from_stop_id` to `to_stop_id`. This field can be used for specifying step-by-step text directions for navigating through the station. It may not be used to refer to the physical signage. <br><br>_Example: "Go downstairs to the station concourse"_|
...
| `reversed_instructions` | Text | Optional | Functions in the same way as `instructions` except applies to instructions when the pathway is taken in the opposite direction, i.e. from `to_stop_id` to `from_stop_id`.|
