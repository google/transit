...
| `cab` | enum | Optional | For rail cars valid options are:<br><br>`0` - No cab in the vehicle.<br>`1` - In addition to a passenger area the car has a cab for controlling the train.  |
...
|  `food` | Enum | Optional | The type of food available. Valid options are:<br><br>`0` - No food available.<br>`1` - Unstaff vending machines with drinks or snacks<br>`2` - Staffed cafe with snacks and hot and cold drinks<br>`3` - Staffed cafe with warmed food and hot and cold drinks<br>`4` - Restraunt service with waiters that bring food to your table. |
