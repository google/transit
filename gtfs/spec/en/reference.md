...
- **Primary ID** - An ID or set of IDs and fields that uniquely identify a row. <br> *Example: the `trip_id` and `stop_sequence` fields make the Primary ID of [stop_times.txt](#stop_timestxt).*
- **Foreign ID** - An ID that references an ID in another .txt file. <br> *Example: The `stop_id` field in [stop_times.txt](#stop_timestxt) is a Foreign ID referencing `stops.stop_id`.*
