...
|  [direction_names.txt](#direction_namestxt) | Optional | Provides direction names for wayfinding purposes. |
...
|  `direction_name_id` | ID referencing `direction_names.direction_name_id` | Optional | Defines a direction name for the trip. The same `trips.direction_name_id` must be provided per combination of `route_id` and `direction_id`. If the `direction_name` changes during a trip, `trips.direction_name_id` can be overwritten by values for the `stop_times.direction_name_id`. |
...
|  `direction_name_id` | ID referencing `direction_names.direction_name_id` | Optional | Defines a direction name from this `stop_time` to the subsequent one. This field overrides the default `trips.direction_name_id` when the `direction_name` changes between stops. If the `direction_name` is displayed for an entire trip, use `trips.direction_name_id` instead. <br><br> A `direction_name_id` value specified for one `stop_time` does not apply to subsequent `stop_time`s in the same trip. If you want to override the `trips.direction_name_id` for multiple `stop_time`s in the same trip, the `stop_times.direction_name_id` must be repeated in each `stop_time` row. |
...
| `direction_name` | Text | **Required** | Text that appears on signage identifying the trip's direction to riders (e.g. "North", "Southbound", "Inbound", "Clockwise"). This text must be used by the transit agency in its wayfinding system. <br><br> A `direction_name` must not replicate the values `trips.trip_headsign` and `stop_times.stop_headsign` provided for the trip, but must be complementary with them. <hr> *Example: If a trip is announced to riders as "Westbound to Hillsboro": "Westbound" should be the `direction_name` and "Hillsboro" should be the `trip_headsign`.* |
