...
* **Readable field** - The field should contain the same information that its parent field (on which it falls back if it is empty). It is aimed to be read as text-to-speech, therefore, abbreviation should be either removed ("St" should be either read as "Street" or "Saint"; "Dr" should be "Drive" or "Doctor") or kept to be read as it ("JFK Airport" is said abbreviated).
