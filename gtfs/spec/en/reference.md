...
| `signposted_as` | Text | Optional | Text from physical signage that is visible to riders when the pathway is taken from `from_stop_id` to `to_stop_id`. The text in this field should appear exactly how it is printed on the physical signage. <br><br> A string can be consumed to provide text directions to users. <br> _Example: "Follow signs to `signposted_as`"._ <br><br> When the physical signage is multilingual, this field may be populated and translated following the example of `stops.stop_name` in the field definition of `feed_info.feed_lang`.|
| `reversed_signposted_as` | Text | Optional | Functions in the same way as `signposted_as` except applies to the physical signage that is visible to riders when the pathway is taken in the opposite direction, i.e. from `to_stop_id` to `from_stop_id`. |
| `visually_impaired_instructions`| Text | Optional | Step-by-step navigation instructions describing a pathway taken from `from_stop_id` to `to_stop_id`. This field is intended for visually-impaired riders and must therefore be readable by text-to-speech software. See "Text-to-speech field" in the [Term Definitions](#term-definitions).<br><br>_Example: "Turn left and go down the corridor."_|
| `reversed_visually_impaired_instructions` | Text | Optional | Functions in the same way as `visually_impaired_instructions` but when the pathway is taken in the opposite direction (i.e., from `to_stop_id` to `from_stop_id`).|
